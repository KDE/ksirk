...
SRCDIR=../ksirk # srcdir is the directory containing the source code
#TIPSDIR=$SRCDIR/appdata # tipsdir is the directory containing the tips
...
$XGETTEXT `find $SRCDIR -name "*.cpp" -o -name "*.h"` rc.cpp -o ksirk.pot
...
for i in `ls *.po`; do
    msgmerge $i ksirk.pot -o $i || exit 1
done
