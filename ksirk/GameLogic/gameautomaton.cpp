...
bool GameAutomaton::setupPlayersNumberAndSkin(bool& networkGame, int& port, uint& newPlayersNumber)
{
  ...
  {
    QString mes = "";
    mes = i18n("Error - 2 nations minimum. Got %d.",nations.size());
...
//   m_game->haltTimer();
  if (m_state != GAME_OVER)
  {
    if (KMessageBox::questionYesNoCancel(m_game,
                                  ...
                                  KGuiItem(i18n("New Game")), KGuiItem(i18n("Exit")), i18n("Do nothing")) == KMessageBox::Yes)
    {
      if (!m_game->actionNewGame())
        exit(0);
    }
...
//     kDebug() << "Setting owner of " << (theWorld()-> getCountries().at(*it)->name()) << " to " << currentPlayer()->name() << endl;
    QByteArray buffer;
    QDataStream stream(&buffer, QIODevice::WriteOnly);
