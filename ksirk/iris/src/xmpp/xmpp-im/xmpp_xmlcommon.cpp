...
{
	const QString str = QString::asprintf("%04d%02d%02dT%02d:%02d:%02d",
		d.date().year(),
		d.date().month(),
		d.date().day(),
		d.time().hour(),
...
{
	const QString str = QString::asprintf("%d,%d", s.width(), s.height());
...
{
	const QString str = QString::asprintf("%d,%d,%d,%d", r.x(), r.y(), r.width(), r.height());
